    ...
    get(): QSelectOption<CityProvinceResponse> | null {
        if (userLocation?.value?.district?.cityProvince?.name === undefined) {
            return userLocationProps?.value?.district?.cityProvince
                ? toCityProvinceOption(
                      userLocationProps.value.district.cityProvince
                  )
                : null;
        }
...
const selectedWardTownVillage = computed({
    get(): QSelectOption<WardTownVillageResponse> | null {
        ...
        return userLocation?.value
            ? toWardTownOption(userLocation.value)
            : null;
    },
    set(selectedWardTownOption) {
        ...
        if (selectedWardTownOption?.value) {
            ...
            newLocation.district.cityProvince =
        ...
        locationStore
            .getDistrictAutocomplete(
                ...
                selectedCityProvince.value
                    ? selectedCityProvince.value.value?.id
                    : ""
            )
            .then((values) => {
                if (values && values.length > 0) {
